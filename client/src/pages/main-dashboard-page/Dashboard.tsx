...
export function Dashboard() {
  ...
  const FilterPanel = () => (
    <motion.div
      ...
      className={cn(
        // Use themed tokens with a touch of transparency for smooth dark mode
        "bg-card/70 dark:bg-card/60 border border-border rounded-xl p-6 shadow-lg transition-all duration-300 backdrop-blur-sm",
      ...
      <div className="space-y-6">
        <div>
          ...
          <DropdownMenu>
            ...
            <DropdownMenuContent className="w-[var(--radix-dropdown-menu-trigger-width)] bg-popover text-foreground border border-border">
        ...
        <div className="space-y-4">
          <div>
            ...
            <Popover>
              ...
              <PopoverContent className="w-auto p-0 bg-popover text-foreground border border-border">
                <DayPicker
                  mode="single"
            ...
            <Card>
              ...
              <CardContent>
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4 max-h-96 overflow-y-auto pr-2">
                  ...
                  const rawDate = (log as TrendData).date as unknown;
                  let date: Date | null = null;
                  if (
                    rawDate &&
                    typeof rawDate === "object" &&
                    "_seconds" in (rawDate as Record<string, unknown>) &&
                    typeof (rawDate as Record<string, unknown>)._seconds === "number"
                  ) {
                    date = new Date((rawDate as { _seconds: number })._seconds * 1000);
  ...
  return (
    <div className="min-h-screen p-4 md:p-8 font-sans">
      ...
      {!isMdUp && filters.isFilterOpen && (
        <div className="fixed inset-0 z-50 flex items-start justify-center p-4 bg-background/80 backdrop-blur-sm">
          <FilterPanel />
        </div>
      )}
    </div>
